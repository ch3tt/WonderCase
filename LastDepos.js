...
status = 'Успешно';
    ...
    document.querySelector('.table').innerHTML = document.querySelector('.table').innerHTML + '<nav><div> <h6>'+id+'</h6> </div><div> <h6>'+date+'</h6> </div><div> <h6>'+type+'</h6> </div><div> <h6>'+sum+'₽</h6> </div><div> <h6 className='+sc+'>'+status+'</h6> </div></nav>';
  }
    axios.get('https://wondercase.net/last10Pays.php').then ((response) => {
      if (did)return;
